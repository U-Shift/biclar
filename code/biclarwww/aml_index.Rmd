...
A linha a tracejado representa os valor comulativo de número de viagens em bicicleta por dia do IMOB, para cada distância. Como se pode observar, o maior potencial de viagens em bicicleta situa-se em viagens até 10 km, sendo que os primeiros 5 km são muito expressivos.
...
TPtable_car = CAR_TP_TRANSF %>%
  ...
  summarise(TP_4 = sum(TP_4), TP_10 = sum(TP_10)
            # Total = sum(Total), Bike = sum(Bike), Car = sum(Car), TP = sum(TP), 
            # Bike_4 = sum(Bike_4), Bike_10 = sum(Bike_10)
            ) %>% 
  ungroup() %>% 
