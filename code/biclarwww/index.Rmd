...
REDEexistente = sf::st_read("https://ushift.tecnico.ulisboa.pt/content/tml/RedeExistente.gpkg", quiet = TRUE) %>%
  sf::st_transform(4326) %>%
  arrange(., desc(Tipologia)) #para as nao dedicadas ficarem em baixo nas intersecções
REDEproposta = sf::st_read("https://ushift.tecnico.ulisboa.pt/content/tml/RedePrevista.gpkg", quiet = TRUE) %>%
  sf::st_transform(4326) %>%
...
Os percursos diretos foram estimados usando como nível de stress máximo permitido o LTS 3.  
...
> Nota 2: Para alguns municípios não é possível estimar viagens intermodais com recurso à bicicleta, para os critérios mínumos definidos (valores _em branco_).
...
Ver o [relatório metodológico](nada) para mais detalhes sobre os métodos e funcionamento desta ferramenta.
...
São eles os municípios de Alcochete, Mafra, Montijo, Odivelas e Sesimbra.
