...
HEAT_car = routes_r5r_100jit_car %>%
  filter(distance <= max_euc) %>% #euclidean distance of routes_ferry3_filtered
...
InTraffic = readRDS(HEAT$rnet_filepath) %>% filter(Bike>0) %>% filter(carspeed > 30)
...
HEAT = HEAT %>% mutate(Mortality = from_hass$results$impactperyearave[30],
  ...
  mutate(value_newcyc = Economic10/Bike_new,
         value_newcyc_eur = Economic10/Bike_new*usd_to_eur)
