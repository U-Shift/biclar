...
%DIF DEL paper/PaperCEUS/PaperCEUS_rev1.tex        Sun May 19 22:07:39 2024
%DIF ADD paper/PaperCEUS/PaperCEUS_rev2_word.tex   Sun Oct  6 01:16:02 2024
...
%DIF 18d18
%DIF <   \linenumbers % turns line numbering on
%DIF -------
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Population distribution in the LMA\label{fig:lma-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev1_files/figure-latex/lma-1} }\subfloat[Trips/day in the LMA\label{fig:lma-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev1_files/figure-latex/lma-2} }%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Population distribution in the LMA\label{fig:lma-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/lma-1} }\subfloat[Trips/day in the LMA\label{fig:lma-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/lma-2} }\DIFaddendFL \caption{\label{lma}Population and daily trips (origins) at each Municipality of the LMA, according to the 2021 census and the 2017 travel survey.}\label{fig:lma}
\end{figure}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[In percentage\label{fig:mododist-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev1_files/figure-latex/mododist-1} }\subfloat[In total\label{fig:mododist-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev1_files/figure-latex/mododist-2} }%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[In percentage\label{fig:mododist-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mododist-1} }\subfloat[In total\label{fig:mododist-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mododist-2} }\DIFaddendFL \caption{\label{mododist}Trips in the LMA by inter/intra municipal and mode, according to the travel survey.}\label{fig:mododist}
\end{figure}
...
\DIFaddbegin \DIFadd{Trips were routed for all the OD jittered pairs, resulting in the
least-cost paths (shortest travel time) for each pair.
}

\DIFadd{Figure \ref{fig:trip_schematic} illustrates a schematic comparison
between }\emph{\DIFadd{safe}} \DIFadd{and }\emph{\DIFadd{direct}} \DIFadd{routing profiles, showing both
bicycle-only and Bike + PT options. In the }\emph{\DIFadd{direct}} \DIFadd{profile,
...
cycling with PT can be faster. In the }\emph{\DIFadd{safe}} \DIFadd{routing profile, where
...
\caption{\DIFaddFL{Schematic trip for safe and direct routing profiles, with bicycle-only and bicycle with PT options. The black lines represent the Biclcye legs, and the dashed orange lines represent the PT leg.}}\label{fig:trip_schematic}
\end{figure}

\DIFaddend The \texttt{r5r} model \DIFdelbegin \DIFdel{used }\DIFdelend \DIFaddbegin \DIFadd{uses }\DIFaddend the OpenStreetMap road network and the GTFS
...
\DIFdelbegin \DIFdel{We restricted our analysis to the
first and last legs with a combined length of up to 5 km (for example: 1
km from origin to interface }\emph{\DIFdel{A}} %DIFAUXCMD
\DIFdel{plus 4 km from interface }\emph{\DIFdel{B}}
%DIFAUXCMD
\DIFdel{to destination). Furthermore, we have imposed restrictions on PT usage,
limiting it to trips without PT transfers, and within a duration of up
to 2 hours (120 minutes). Additionally, we }\DIFdelend \DIFaddbegin 

\DIFadd{We }\DIFaddend have only included PT modes that can easily accommodate bicycles,
...
\DIFaddbegin \DIFadd{Furthermore, we have imposed restrictions on PT usage, limiting it to
...
to interface }\emph{\DIFadd{A}} \DIFadd{plus 4 km from interface }\emph{\DIFadd{B}} \DIFadd{to destination).
}

\DIFaddend This conservative approach\DIFaddbegin \DIFadd{, regarding the assumed multi-modal
restrictions, }\DIFaddend was adopted to capture the fact that cycling stages as
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\linewidth,]{PaperCEUS_rev1_files/figure-latex/map1-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/map1-1} 
\DIFaddendFL 
...
impacts of shifting car trips to PT (between the PT interfaces). \DIFaddbegin \DIFadd{Car
...
distance \DIFdelbegin \DIFdel{traveled }\DIFdelend \DIFaddbegin \DIFadd{travelled }\DIFaddend per trip influences the overconsumption and emissions
...
different route profiles, at an aggregate level of the LMA. \DIFaddbegin \DIFadd{It should be
...
\DIFaddend For the cycling legs of the journey (first and last legs), the
...
\caption{\label{tab:summary1}\label{summary1}Summary of the cycling \DIFdelbeginFL \DIFdelFL{potencial }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{potential }\DIFaddendFL of the intermodality scenario. Values in 'trips/day'.}
\centering
\begin{tabular}[t]{llr>{\raggedleft\arraybackslash}p{7em}>{\raggedleft\arraybackslash}p{7em}}
\toprule
Target & Routing & Total trips & Baseline Cycling + PT & \DIFdelbeginFL \DIFdelFL{Potencial }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Potential }\DIFaddendFL Cycling + PT\\
\midrule
...
\caption{\label{tab:summary1b}\label{summary1b}Summary of the cycling \DIFdelbeginFL \DIFdelFL{potencial }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{potential }\DIFaddendFL of intermodality scenario and its socio-environmental benefits for the cycling legs.}
\centering
...
\DIFdelbegin \DIFdel{For both }\emph{\DIFdel{direct}} %DIFAUXCMD
\DIFdel{and }\emph{\DIFdel{safe}} %DIFAUXCMD
\DIFdel{route profiles, 10\% of the daily
trips have the potential to be made by a combination of PT and cycling,
even }\DIFdelend \DIFaddbegin \DIFadd{Even }\DIFaddend given the travel restrictions considered (up to 5 km on bike, up to
2 hours, no possible transfers between PT)\DIFaddbegin \DIFadd{, this resulted in 538.514
trips with potential to be made by bicycle in combination with PT
(10.1\% of all daily trips) for the }\emph{\DIFadd{safe}} \DIFadd{routing profile, and
500.880 trips for the }\emph{\DIFadd{direct}} \DIFadd{routing profile (9.4\% of all daily
trips)}\DIFaddend . This unveils the potential of cycling as a complementary mode of
...
\caption{\label{tab:summary21}\label{summary21}Summary of the potential of replacing car trips with cycling in combination with PT, \DIFdelbeginFL \DIFdelFL{disagregated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{disaggregated }\DIFaddendFL by PT mode. Values in 'trips/day'.}
\centering
...
\DIFaddbegin \DIFadd{The higher number of Bike + PT trips in the }\emph{\DIFadd{safe}} \DIFadd{routing profile
...
the }\emph{\DIFadd{direct}} \DIFadd{routing profile, which allows for direct cycling
...
}\emph{\DIFadd{safe}} \DIFadd{routing profile, while }\emph{\DIFadd{direct}} \DIFadd{routes favour
cycling-only journeys. This justifies the aggregate difference in the
number of potential trips between the two profiles.
}

\DIFadd{For trams, which primarily operate along busy main roads that are
...
\ref{fig:map1}). For }\emph{\DIFadd{direct}} \DIFadd{routing, bike-only options are
usually faster for shorter local trips within the municipality. However,
under }\emph{\DIFadd{safe}} \DIFadd{routing assumptions (LTS 3), where cyclists are
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \subfloat[Potential new Bicycle + PT trips\label{fig:mapzones-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev1_files/figure-latex/mapzones-1} }\subfloat[Proportion of total potential modal shift in the LMA\label{fig:mapzones-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev1_files/figure-latex/mapzones-2} }%%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Potential new Bicycle + PT trips\label{fig:mapzones-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mapzones-1} }\subfloat[Proportion of total potential modal shift in the LMA\label{fig:mapzones-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mapzones-2} }\DIFaddendFL \caption{\label{mapzones}Potential Bicycle + PT daily trips for the 10\% target and direct routing, by municipality (origins). The percentages are shown for municipalities with values above 4\%.}\label{fig:mapzones}
\end{figure}
