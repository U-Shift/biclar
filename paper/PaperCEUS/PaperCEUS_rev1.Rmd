...
```{r mapzones, fig.cap="Potential Bike + PT daily trips for the 10% target, by municipality"}
...
Consequently, operators and public space managers would face the challenge of accommodating this increased demand for bicycle parking and/or docks from a bike sharing system.
...
## Model replicability and complexity

This analysis can be applied to other metropolitan areas, providing a basis for decision-making and policy development to support a modal shift from car to PT. This process involves a minimum of data requirements, such as the origin-destination data, road network, and the PT GTFS data, which are available in most metropolitan areas.
...
<!-- CONTINUE HERE -->
A further refinement could be to differentiate between ‘simpler’ no-transfer PT trips (which require less person effort) and more arduous multi-transfer PT trips, using the same methods: we could set a different maximum cycling stage distance for the former and a shorter maximum cycling stage distance for the latter. However, this would have an impact on the explainability of the results to policy makers and the overall complexity.
We now discuss this and other potential refinements to the methods using the same input datasets and open, reproducible methods in the discussion with reference to the **likelihood of diminishing returns with additional model complexity**.
