...
Trips were routed for all the OD jittered pairs, using `r5r`.
The resulting estimated routes are the ones with the least-cost paths (shortest travel time) for each OD pair.
For some routes, bicycle alone can be more attractive than the combination of bicycle and PT - for instance for shorter trips.

Figure \ref{fig:trip_schematic} illustrates a schematic trip for different routing profiles (_safe_ and _direct_), and only using Bicycle or Bicycle in combination with Public Transportation, considering the same origin and destination.
As we can see, for the same routing profile, the _direct_ route by bicycle can be more convenient (attractive) than the bicycle in combination with PT.
The opposite can also be true, in particular for longer journeys, where the combination of bicycle and PT can be more attractive than the bicycle mode.
When comparing the _direct_ with the _safe_ route profile, the _safe_ route is usually longer, as it avoids the most stressful roads (LTS 4), which are usually the most direct ones, considering only roads with LTS 3 or lower.
In this case, bike (without combination) can be more attractive than the bike with PT, as the PT leg can be longer or take more time than the safe bike route.

```{r trip_schematic, fig.align='center', fig.cap="Schematic trip for different routing profiles (safe and direct), and only using Bicycle or Bicycle with Public Transportation, considering the same origin and destination. The black lines represent the trip made by Bike, and the dashed orange lines represent the trip made by PT.", fig.height=4, out.width="70%"}
...
This resulted in that only 53.8% of the total trips between the jittered origins and destinations are possible to be made by bicycle or bicycle + PT (for the considered PT modes) for the _safe_ routing profile, and 69.2% for the _direct_ routing profile.
...
This process resulted in a subset of the OD pairs used in the intermodality scenario: 19.5% of the total OD pairs with _direct_ routing profile, and 21% of the total OD pairs with _safe_ routing profile.
This difference is due to the fact that the direct routes can be more attractive for cycling only than for the combination of bicycle and PT.
In particular, because main PT connections are along busy roads, and thus, for short trips (e.g. intramunicipal trips) using bicycle only at the main roads can be more attractive (shorter travel time) than the combination of bicycle and PT.
For _safe_ routes, the alternative of using bicycle for the PT leg using safe routes can take longer - since the main roads are restricted in this routing profile -, and thus less attractive.
...
This restriction resulted in 538.514 trips that can be made by bicycle in combination with PT (`r round(538514/sum(TRIPSmode_aml$viagens)*100,1)`% of all daily trips) considering the safe routing profile, and 500.880 trips considering the direct routing profile (`r round(500880/sum(TRIPSmode_aml$viagens)*100,1)`% of all daily trips).
