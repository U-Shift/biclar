...
\begin{figure}
\subfloat[Population distribution in the LMA\label{fig:lma-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/lma-1} }\subfloat[Trips/day in the LMA\label{fig:lma-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/lma-2} }\caption{\label{lma}Population and daily trips (origins) at each Municipality of the LMA, according to the 2021 census and the 2017 travel survey.}\label{fig:lma}
\end{figure}
...
\begin{figure}
\subfloat[In percentage\label{fig:mododist-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mododist-1} }\subfloat[In total\label{fig:mododist-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mododist-2} }\caption{\label{mododist}Trips in the LMA by inter/intra municipal and mode, according to the travel survey.}\label{fig:mododist}
\end{figure}
...
is used for the first and last legs. The trips were routed for all the
OD jittered pairs, using \texttt{r5r}. The estimated routes are the ones
with the least-cost paths (shortest travel time). For some routes,
bicycle alone can be more attractive than the combination of bicycle and
PT - for instance for shorter trips.

Figure \ref{fig:trip_schematic} illustrates a schematic trip for
different routing profiles (safe and direct), and only using Bicycle or
Bicycle in combination with Public Transportation, considering the same
origin and destination. As we can see, for the same routing profile, the
direct route by bicycle can be more convenient (attractive) than the
bicycle in combination with PT. The opposite can also be true, in
particular for longer journeys, where the combination of bicycle and PT
can be more attractive than the bicycle mode. When comparing the direct
with the safe route profile, the safe route is usually longer, as it
avoids the most stressful roads (LTS 4), which are usually the most
direct ones, considering only roads with LTS 3 or lower. In this case,
the bike route (without combination) can be more attractive than the
bike with PT, as the PT leg can be longer or take more time than the
safe bike route.
...
\caption{Schematic trip for different routing profiles (safe and direct), and only using Bicycle or Bicycle with Public Transportation, considering the same origin and destination. The black lines represent the trip made by Bike, and the dashed orange lines represent the trip made by PT.}\label{fig:trip_schematic}
\end{figure}
...
minutes). This resulted in that only 53.8\% of the total trips between
the jittered origins and destinations are possible to be made by bicycle
or bicycle + PT (for the modelled PT modes), considering the safe
routing profile, and 69.2\% considering the direct routing profile.

After the routes were estimated, we considered only the
Origin-Destination pairs for which the estimated routes were the ones
that were more attractive (least-cost path / fastest) for the bicycle
and PT combination, than for bicycle only. This process resulted in a
subset of the OD pairs that were considered for the intermodality
scenario: 19.5\% of the total OD pairs with direct routing profile, and
21\% of the total OD pairs with safe routing profile. This difference is
due to the fact that the direct routes are can be more attractive for
the bicycle mode than the combination of bicycle and PT, in particular
because main PT connections are along busy roads, and thus, for short
trips (for instance intramunicipal trips) the bicycle mode can be more
attractive than the combination of bicycle and PT. For safe routes, the
alternative of using bicycle for the PT leg using safe routes can take
longer - since the main roads are restricted in this routing profile -,
and thus less attractive.

Additionally, we restricted our analysis to the first and last legs with
a combined length of up to 5 km (for example: 1 km from origin to
interface \emph{A} plus 4 km from interface \emph{B} to destination).
This restriction resulted in 538.514 trips that can be made by bicycle
in combination with PT (10.1\% of all daily trips) considering the safe
routing profile, and 696.000 trips considering the direct routing
profile (9.4\% of all daily trips).
...
A particular difference is noted regarding the attraction of trams to be
used in combination with cycling, for \emph{safe} and \emph{direct}
routes. In this case, Tram is available for intermodality only at one
municipality (Almada) (see \ref{fig:map1}), and most of its stations are
located on major roads. Their accessibility involves travelling along
these major roads (with LTS 4), which limits the potential of valid
routing options when considering \emph{safe} routes. In this case, Train
offers the highest potential for \emph{safe} routes, and Tram for
\emph{direct} routes.
...
\begin{figure}
\subfloat[Potential new Bicycle + PT trips\label{fig:mapzones-1}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mapzones-1} }\subfloat[Proportion of total potential modal shift in the LMA\label{fig:mapzones-2}]{\includegraphics[width=0.5\linewidth,]{PaperCEUS_rev2_word_files/figure-latex/mapzones-2} }\caption{\label{mapzones}Potential Bicycle + PT daily trips for the 10\% target and direct routing, by municipality (origins). The percentages are shown for municipalities with values above 4\%.}\label{fig:mapzones}
\end{figure}
