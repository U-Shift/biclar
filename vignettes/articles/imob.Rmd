...
tabela = tabela %>% select(Dicofre, Concelho, Freguesia, PopFreg, error, margem_erro) %>% mutate(error = round(error*100,2)) %>% arrange(as.integer(tabela$Dicofre))
...
-   We don't know how many survey respondents were by _Freguesia_. From the Mobility Survey database, we can only figure out their home Municipality and Zone. For this purpose, we considered the _Freguesia_ corresponding with the destination of their stated trip purpose "Return home".
